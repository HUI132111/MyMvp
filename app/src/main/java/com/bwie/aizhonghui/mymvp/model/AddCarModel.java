...
public class AddCarModel {
    ...
    public AddCarModel(Activity context) {
    ...
    public void addCar(Map<String,String> map){
        ...
        final Request request=new Request.Builder().post(body).url(Api.SHOP_ADDCAR_API).build();
        Call call = okHttpClient.newCall(request);
        call.enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                iAddCar.AddcarError(call,e);
            }
                ...
                if(response!=null&&response.isSuccessful()){
                    ...
                    System.out.println("======"+result);
                        context.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                iAddCar.AddcarSuccess("6",result);
                            }
                        });
    ...
    public  void setiAddCar(IAddCar iAddCar){
